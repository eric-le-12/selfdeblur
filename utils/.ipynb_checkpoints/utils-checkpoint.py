...
def ssim_loss(x, y, device,window_size=11):
    """
    ...
    denominator = (mu_x * mu_x + mu_y * mu_y + const1)*(sigma_x_sq + sigma_x_y_sq + const2)
    ssim = (numerator / denominator).mean()
    return 1 - ssim


def generate_input(shape, scale=0.1, ntype="uniform"):
    """Generate input tensor of size shape, which is then multiplied by a scale_factor
    type can be either normal or uniform
    """
    if ntype == "uniform":
        return torch.zeros(shape).uniform_() * scale
    else:
        # normal
