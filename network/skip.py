...
def skip(
    ...
    for i in range(len(num_channels_down)):
        ...
        model_tmp.add(layers.bn(num_channels_skip[i] + (num_channels_up[i + 1] if i < last_scale else num_channels_down[i])))
        # skip.add(Concat(2, GenNoise(nums_noise[i]), skip_part))
        

        if num_channels_skip[i] != 0:
            skip.add(layers.conv(input_depth, num_channels_skip[i], filter_skip_size, bias=need_bias, pad=pad))
            skip.add(layers.bn(num_channels_skip[i]))
            skip.add(act(act_fun))
            
        #### debugging
  

        deeper.add(layers.conv(input_depth, num_channels_down[i], filter_size_down[i], 2, bias=need_bias, pad=pad, downsample_mode=downsample_mode[i]))
        deeper.add(layers.bn(num_channels_down[i]))
        deeper.add(act(act_fun))
        if i>1:
            deeper.add(block.NONLocalBlock2D(in_channels=num_channels_down[i]))
        deeper.add(layers.conv(num_channels_down[i], num_channels_down[i], filter_size_down[i], bias=need_bias, pad=pad))
        deeper.add(layers.bn(num_channels_down[i]))
        deeper.add(act(act_fun))
        ...
        model_tmp.add(layers.conv(num_channels_skip[i] + k, num_channels_up[i], filter_size_up[i], 1, bias=need_bias, pad=pad))
        model_tmp.add(layers.bn(num_channels_up[i]))
        model_tmp.add(act(act_fun))


        if need1x1_up:
            model_tmp.add(layers.conv(num_channels_up[i], num_channels_up[i], 1, bias=need_bias, pad=pad))
            model_tmp.add(layers.bn(num_channels_up[i]))
            model_tmp.add(act(act_fun))
    ...
    model.add(layers.conv(num_channels_up[0], num_output_channels, 1, bias=need_bias, pad=pad))
    if need_sigmoid:
